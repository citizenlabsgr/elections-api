...
class Ballot(TimeStampedModel):
    ...
    def _parse_nonpartisan_section(self, data):
        for category_name, positions_data in data.items():
            if category_name in {'Community College'}:
                # TODO: Verify this is the correct mapping for 'Community College'
                district = self.precinct.county
            elif category_name in {'City', 'Township', 'Village', 'Local School'}:
