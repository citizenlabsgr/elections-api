...
def check_repeated(text):
        ...
        for other_index in range(index + 1, len(list_of_words)):
            if word == list_of_words[other_index]:
                repeated_list.append(
                    ((index, word), (other_index, list_of_words[other_index]))
                )
                break
    ...
    for index, repeated_tuple in enumerate(repeated_list):
        if initial_tuple is None:
            ...
            len(repeated_list) > index + 1
            and repeated_tuple[0][0] + 1 == repeated_list[index + 1][0][0]
        ):
            consecutive_count += 1
        elif (
            consecutive_count > 0
            ...
            bugsnag.notify(ValueError("duplicate value found"))
            for index in range(
                initial_tuple[0][0], initial_tuple[0][0] + consecutive_count + 1
            ):
